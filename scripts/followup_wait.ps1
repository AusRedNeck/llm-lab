...
cmd /c "python -u scripts\tokenize_owt_16k.py --workers 8 --status-every 60 >> `"$runlog`" 2>&1"
$rc = $LASTEXITCODE
Write-Marker "encode pass exited $rc"

if ($rc -eq 0) {
    cmd /c "python -u scripts\verify_tokens.py --bin data/openwebtext_combined_bpe_owt16k.bin --vocab data/bpe_owt16k.json --src data/openwebtext/shards/train-00000-of-00080.txt --manifest >> `"$runlog`" 2>&1"
    $vrc = $LASTEXITCODE
    if ($vrc -eq 0) {
