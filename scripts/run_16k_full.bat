...
python -u scripts\tokenize_owt_16k.py >> logs\16k_tokenize.log 2>&1
echo === encode finished %date% %time% === >> logs\16k_tokenize.log

python -u scripts\verify_tokens.py --bin data/openwebtext_combined_bpe_owt16k.bin --vocab data/bpe_owt16k.json --src data/openwebtext/shards/train-00000-of-00080.txt --manifest >> logs\16k_tokenize.log 2>&1
